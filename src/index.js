...
function getAddress(addressInput) {
  ...
  if (typeof(address) !== 'string') { throw new Error(`[vapjs-account] invalid address value ${JSON.stringify(address)} not a valid hex string`); }
...
function privateToPublic(privateKey) {
  if (typeof privateKey !== 'string') { throw new Error(`[vapjs-account] private key must be type String, got ${typeof(privateKey)}`); }
  if (!privateKey.match(/^(0x)?[0-9a-fA-F]{64}$/)) { throw new Error('[vapjs-account] private key must be an alphanumeric hex string that is 32 bytes long.'); }
...
function publicToAddress(publicKey) {
  if (!Buffer.isBuffer(publicKey)) { throw new Error('[vapjs-account] public key must be a buffer object in order to get public key address'); }
...
function generate(entropy) {
  if (typeof entropy !== 'string') { throw new Error(`[vapjs-account] while generating account, invalid input type: '${typeof(entropy)}' should be type 'String'.`); }
  if (entropy.length < 32) { throw new Error(`[vapjs-account] while generating account, entropy value not random and long enough, should be at least 32 characters of random information, is only ${entropy.length}`); }
